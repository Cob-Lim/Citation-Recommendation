...
q_template = (
    "Citation context:\n"
    "{citation_context}\n\n"
    "The query above came from a research paper in the ACL Anthology.\n\n"
...
q_prompt = PromptTemplate(
    input_variables=["citation_context"],
    template=q_template
)
...
for i in range(len(masked_contexts)):
    filled_prompt = q_prompt.format(citation_context=masked_contexts[i])
    ...
    if (i + 1) % 25 == 0:
        ...
        with open("QtoC_text-responses.json", "w") as f:
            json.dump(text_responses, f, indent=4)
        print(f"Saved {i+1} text responses to QtoC_text-responses.json")

        with open("QtoC_search-proofs.json", "w") as f:
            json.dump(search_proofs, f, indent=4)
        print(f"Saved {i+1} search proofs to QtoC_search-proofs.json")
...
with open("QtoC_text-responses.json", "w") as f:
    json.dump(text_responses, f, indent=4)
print(f"Saved all text responses to QtoC_text-responses.json")

with open("QtoC_search-proofs.json", "w") as f:
    json.dump(search_proofs, f, indent=4)
print(f"Saved all search proofs to QtoC_search-proofs.json")