...
q_template = (
    ...
    "{target_abstract}\n\n"
    "The query and abstract above comes from a research paper in the ACL Anthology titled \"{target_title}\".\n\n"
...
q_prompt = PromptTemplate(
    input_variables=["citation_context", "target_title", "target_abstract"],
    template=q_template
)
...
target_abstracts = df["target_abstract"].tolist()
...
for i in range(len(masked_contexts)):
    filled_prompt = q_prompt.format(
        ...
        target_title=target_titles[i],
        target_abstract=target_abstracts[i]
    )
    ...
    if (i + 1) % 25 == 0:
        ...
        with open("QTTTAtoC_text-responses.json", "w") as f:
            json.dump(text_responses, f, indent=4)
        print(f"Saved {i+1} text responses to QTTTAtoC_text-responses.json")

        with open("QTTTAtoC_search-proofs.json", "w") as f:
            json.dump(search_proofs, f, indent=4)
        print(f"Saved {i+1} search proofs to QTTTAtoC_search-proofs.json")
