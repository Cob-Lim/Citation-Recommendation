...
q_template = (
    "Citation context:\n"
    "{citation_context}\n\n"
    "The query above came from a research paper in the ACL Anthology titled \"{target_title}\".\n\n"
...
q_prompt = PromptTemplate(
    input_variables=["citation_context", "target_title"],
    template=q_template
)
...
for i in range(len(masked_contexts)):
    ...
    if (i + 1) % 25 == 0:
        ...
        with open("QTTtoC_text-responses.json", "w") as f:
            json.dump(text_responses, f, indent=4)
        print(f"Saved {i+1} text responses to QTTtoC_text-responses.json")

        with open("QTTtoC_search-proofs.json", "w") as f:
            json.dump(search_proofs, f, indent=4)
        print(f"Saved {i+1} search proofs to QTTtoC_search-proofs.json")
