...
q_template = (
    ...
    "{citing_abstract}\n\n"
    "The query and abstract above comes from a research paper in the ACL Anthology titled \"{citing_title}\".\n\n"
...
q_prompt = PromptTemplate(
    input_variables=["citation_context", "citing_title", "citing_abstract"],
    template=q_template
)
...
citing_abstracts = df["citing_abstract"].tolist()
...
for i in range(len(masked_contexts)):
    filled_prompt = q_prompt.format(
        ...
        citing_abstract=citing_abstracts[i]
    )
    ...
    if (i + 1) % 25 == 0:
        ...
        with open("QCTCAtoC_text-responses.json", "w") as f:
            json.dump(text_responses, f, indent=4)
        print(f"Saved {i+1} text responses to QCTCAtoC_text-responses.json")

        with open("QCTCAtoC_search-proofs.json", "w") as f:
            json.dump(search_proofs, f, indent=4)
        print(f"Saved {i+1} search proofs to QCTCAtoC_search-proofs.json")
