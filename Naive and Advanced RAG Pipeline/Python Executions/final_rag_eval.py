...
def rerank_results(query, retrieved_citation_items, retrieved_chunks, retrieved_titles, reranker_type, reranker_model, top_k):
...
def match_citations(query, retrieved_citation_items, retrieved_chunks, retrieved_titles, reranker_model, rerank, reranker_type, citation_target, top_k=10):
    """
    Matches citations.
    ...
    if rerank == True:
        unique_citation_items = rerank_results(query, retrieved_citation_items, retrieved_chunks, retrieved_titles, reranker_type, reranker_model, top_k)
    else:
...
def compute_all_metrics_batches(csv_path, just_context, chunks_file_path, citations_file_path, titles_file_path, rerank, reranker_type, reranker_model, top_k, batch_size=100):
    """
    ...
    for batch_start in range(0, len(all_queries), batch_size):
        ...
        batch_titles = retrieved_titles[batch_start:batch_start + batch_size]

        for query, citation_target, chunk, citation, title in zip(batch_queries, batch_targets, batch_chunks, batch_citations, batch_titles):
...
def main(num_of_chunks, num_of_overlaps, number_of_docs, top_k, rerank, reranker_type):
    ...
    print("Chunks, Citations, and Titles file path loaded.")
    ...
    if rerank == False:
        ...
        reranker_model = CrossEncoder("mixedbread-ai/mxbai-rerank-large-v1", device=device) # pass any reranker to make it work

    # Define reranker model
    if reranker_type == "flag":
        reranker_model_name = "BAAI/bge-reranker-v2-minicpm-layerwise"
        reranker_model = LayerWiseFlagLLMReranker(reranker_model_name, use_fp16=True, trust_remote_code=True, device=device) # Setting use_fp16 to True speeds up computation with a slight performance degradation
    ...
    print(f"Evaluating for {just_context}_Query_Only-{rerank}_Rerank-{num_of_chunks}-{num_of_overlaps}-Final RAG Pipeline-{number_of_docs}-top_{top_k}")
    ...
    overall_metrics = compute_all_metrics_batches(
        ...
        titles_file_path=titles_file_path, # Titles file path
    ...
    print(f"Done Evaluating for {just_context}_Query Only-{rerank}_Rerank-{num_of_chunks}-{num_of_overlaps}-Final RAG Pipeline-{number_of_docs}-top_{top_k}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reranker_type", type=str, default="mxbai_v1", 
                        help="Type of reranker to use (mxbai_v1 or mxbai_v2).")
